...
const dividendsControllerAddress = 'TQ57ZhTdv61BUQtk27X8syiz7ZC86Wupyw';
const dividendsRequestDataAddress = 'TPFeLXUXyiojk47Qm8xPwwwpBegok17S1u';
...
const rouletteAddress = 'TNdanVKzBBpoWQ3CMZs9hV5snAVukPHWQe';

const games = [cardsAddress, wheelAddress, slotAddress, rouletteAddress];
...
window.onload = async function () {
    ...
    window.soundManager = app.soundManager = new SoundManager(!localStorageGet('soundDisable'));
    $(window).trigger('SOUND_MANAGER_ACTIVATED');

    if (!app.soundManager.isEnable) {
    ...
    $('.totalPaidOut').html(numberWithCommas(money(await getDividendsSum(), 0)) + ' TRX');


};

async function getDividendsSum() {

    const dividendsController = await getContract(dividendsControllerAddress);
    const dividendsData = await getContract(dividendsDataAddress);
    const currentLevel = (await dividendsController.getCurrentLevel().call()).toNumber();

    let sum = 0;

    for (let level = 0; level < currentLevel; level++) {
        const dividends = (await dividendsData.getLevelToDividends(level).call()).toNumber();
...
function money(value, f) {
    return ((value.toNumber ? value.toNumber() : value) / 1000000).toFixed(f === undefined ? 2 : f);
}

function requestUnfreezeAll() {
    $('#requestInput').val(money(playerFrozen));

}

function onRequestUnfreezeSend() {
    let value = parseFloat($('#requestInput').val()) * 1000000;

    if (value > app.playerFrozen) value = app.playerFrozen;
    $('#requestInput').val(money(value));


    getContract(dividendsControllerAddress, true).then(
        dividendsControllerTronlink => {

            log('requestUnfreezeTokens', value);

            dividendsControllerTronlink.requestUnfreezeTokens(value).send({feeLimit: 50 * 1000000}).then(res => {
                log('requestUnfreezeTokens', res);

                $('#requestModal').modal('hide');

            }).catch(err => {
                logError('onRequestUnfreezeSend', err);
                alert(err);
            });
        }
    )
            ...
            dividendsController.getCurrentLevel().call()
                .then(level => {
                    ...
                    if (getTronlinkAddress()) {
                        ...
                        getContract(dividendsRequestDataAddress).then(dividendsRequestData => {
                            dividendsRequestData.getPlayerToRequestAmount(getTronlinkAddress()).call()
                                .then(amount => {
                                    dividendsController.getPlayerToRequestTimeEstimate(getTronlinkAddress()).call()
                                        .then(time => {

                                            if (app.requestUnfreezeLastTime === time.toNumber()) {
                                                app.requestUnfreezeDelta--;
                                            } else {
                                                app.requestUnfreezeLastTime = time.toNumber();
                                                app.requestUnfreezeDelta = 0;
                                            }

                                            const correctTime = time.toNumber() + app.requestUnfreezeDelta;
                                            $('.requestUnfreezeTime').html(
                                                ((correctTime > 0 && correctTime < 86400 && amount > 0) ? (elapsedTimeToString(correctTime, true) + '<br>') : '') +
                                                money(amount) + ' Token 888'
                                            );

                                            $('.buttonUnfreeze').toggleClass("disabled", (correctTime >= 0 || amount === 0));
                        ...
                        dividendsData.getPlayerToFrozenAmount(getTronlinkAddress()).call()
                            .then(playerFrozen => {
                                ...
                                app.playerFrozen = playerFrozen.toNumber();
                                ...
                                dividendsData.getLevelToDividends(_level).call()
                                    .then(dividends => {
                                        $('.dividendsSum').html('<a href="https://tronscan.org/#/contract/TGtGhthzyLBYPUKDysXX1YSgRKPYVTQuMe"  target="_blank">' +
                                            money(dividends) + ' TRX'
                                            + '</a>'
                                        );
                                ...
                                getContract(tokenAddress).then(token => {
                                    token.balanceOf(getTronlinkAddress()).call().then(balance => {
                                        ...
                                        $('.Token888Count').html(numberWithCommas((balance.toNumber() + playerFrozen.toNumber()) / 1000000));
                ...
                if (app.myBalance && app.myBalance < newBalance) {
                    ...
                    app.tweenMyBalance = new TWEEN.Tween(app)
                        ...
                        .onUpdate(() => {
                            ...
                            $('.myBalance1').html(numberWithCommas(app.myBalance) + '&nbsp;TRX');
...
function elapsedTimeToString(t, hideDays) {
    const seconds = t % 60;
    ...
    return (hideDays ? '' : (d(days) + ":")) + d(hours) + ":" + d(minutes) + ":" + d(seconds);
}

function updateTopTable() {
    setSideTableData($('#sideTable > tbody:last'), app.gameState.listTopBetSum);
    setSideTableData2(app.gameState.listTopBetSum);
...
function updateMyHistory() {
    const bets = app.gameState.listPlayerBets[app.currentGameIndex];
    if (app.currentGameIndex === 0) {
        setHistoryTableData0($('.history-table-0 > div:last'), bets);
    } else if (app.currentGameIndex === 1) {

        log(app.currentGameIndex)
        setHistoryTableData1($('.history-table-1 > div:last'), bets);
    } else if (app.currentGameIndex === 3) {

        window.roulette.numbers = bets.slice(Math.max(0, bets.length - 20), bets.length).map(bet => bet.winValue).reverse();
...
function updateTables() {
    ...
    if (app.betsCount) {
        ...
        app.tweenBetsCount = new TWEEN.Tween(app)
            ...
            .onUpdate(() => {
                $('.betsCount').html(numberWithCommas(Math.round(app.betsCount)));
            })
            .start();
        ...
        app.tweenTotalWon = new TWEEN.Tween(app)
            ...
            .onUpdate(() => {
                $('.totalWon').html(numberWithCommas(Math.round(app.totalWon)) + ' TRX');
            })
            .start();
    } else {
        ...
        $('.betsCount').html(numberWithCommas(newBetCount));
        $('.totalWon').html(numberWithCommas(newTotalWon) + ' TRX');
    }
}
...
function setTableData(table, data) {
    ...
    for (var i = data.length - 1; i >= 0; i--) {
        ...
        if (gameIndex === 3) {
            betValue = bet.betValues.split(',').map(id => window.classes.DataNames.getName(id)).join(',');

            //log(bet.betValues + ' -> ' + betValue);
            winValue = bet.winIndex;
        }
        //  if (bet.winValue < 21) {
        rows +=
            '<tr>' +
...
function setSideTableData(table, data) {
    ...
    for (var i = 0; i < 20; i++) {
        ...
        rows +=
            `<div class="leader">
                <div>${numberWithCommas(player.sum / 1000) + 'K TRX'}</div>
                <div>${'lv. ' + level(player.sum) + ' ' + addressToShort(player.player)}</div>
            </div>`;
    }

    $('.WageredListTable').html(rows);
}

function setSideTableData2(data) {
    const n = Math.min(data.length, 20);
    ...
    for (var i = 0; i < n; i++) {
        ...
        rows +=
            `<div class="leader"><div>${numberWithCommas(player.sum / 1000) + 'K TRX'}</div><div>${addressToShort(player.player)}</div></div>`;
    }

    // $('.WageredList').html(rows);
}
...
function setHistoryTableData0(table, data) {
    ...
    for (var i = 0; i < 30; i++) {
        let bet = data[i];
        rows +=
            '<div class="item col-2 col-md-4 d-flex align-items-center justify-content-center">' +
            (i < data.length ? cardTypeHistory(bet.winValue, (i < data.length && bet.winAmount > 0 ? 'good' : '')) : ' ')
            + '</div>';
    }

    $('.history-table-0n > div:last').html(rows);
}
...
function setHistoryTableData1(table, data) {
    ...
    for (var i = 0; i < 30; i++) {
        let bet = data[i];
        rows +=
            '<div class="item col-2 col-md-4 d-flex align-items-center justify-content-center ' + (i < data.length && bet.winAmount > 0 ? 'good' : '') + '">' +
            (i < data.length ? bet.winValue : ' ')
            + '</div>';
    }

    $('.history-table-1n > div:last').html(rows);
}
...
function onWithdrawTrx() {
    if (!getTronlinkAddress()) {
        $('#tronLinkModal').modal();
    } else {
        getContract(dividendsControllerAddress, true).then(
            dividendsControllerTronlink => {
                dividendsControllerTronlink.withdrawDividends().send({feeLimit: 50 * 1000000}).then(res => {
                    log('withdrawDividends', res);
                }).catch(err => {
                    logError('withdrawDividends', err);
                    alert(err);
                });
            }
        )
...
window.common.getTronlinkAddress = getTronlinkAddress;
window.common.app = app;
...
$('.nav-tabs a[href="#gameRoulette"]').on('shown.bs.tab', function (e) {
    app.currentGameIndex = 3;
...
function createWheelView() {
    ...
    function onResize() {
        if (lastCanvasWidth === canvas.width() && lastCanvasHeight === canvas.height()) return;
