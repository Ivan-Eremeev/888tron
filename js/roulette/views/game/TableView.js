!function() {
    ...
    class TableView {
        ...
        _initCells() {
            ...
            for (let i = 0; i < ids.length; i++) {
                ...
                let cell = new CellView(this._canvas, this._imagesModel, cellModel, cellModel.horizontal, this._chipsNavigationModel, this._stateModel, 'table', 'horizontal');
        ...
        _drawTable() {
            let image = this._imagesModel.getImage('img/roulette/table/horizontal/table.svg');
            this._scale = TableView.SIZE / image.height;
            let imageWidth = image.width * this._scale;
            let imageHeight = TableView.SIZE;
            this._x = (this._canvas.canvas.width - imageWidth) / 2;
            this._y = this._canvas.canvas.height - TableView.SHIFT - imageHeight;
        ...
        _outCell() {
            ...
            this._over = false;
            $(this).trigger(TableView.OUT_EVENT);
        }
        ...
        _onCellOver(event) {
            this._over = true;
            $(this).trigger(TableView.OVER_EVENT, event.target);
        }
        ...
        mouseClick(parentX, parentY) {
            ...
            let y = (parentY - this._y) / this._scale;
            let cell = this._getCellUnderCursor(x, y);
            if (cell) $(this).trigger(TableView.BET_EVENT, cell);
        }
    ...
    window.classes.TableView = TableView;
}();